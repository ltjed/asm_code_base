...
class biastPC(nn.Module):
    def __init__(self, control_size, hidden_size, output_size, nonlin='tanh', label_modifier = 1):
        # Ed: a variant of tPC that uses a constant u to serve as a bias/to steer hidden representation towards certain provided labels (e.g., one-hot vectors) 
        # for simplicity, current implementation expect control_size = hidden_size
        """A more concise and pytorchy way of implementing tPC

        Suitable for image sequences
        """
        super(biastPC, self).__init__()

        # Ensure control_size matches hidden_size for an identity matrix
        assert control_size == hidden_size, "control_size must be equal to hidden_size for an identity matrix."
        self.label_modifier = label_modifier
        self.hidden_size = hidden_size
        # self.Win = nn.Linear(control_size, hidden_size, bias=False)

        self.Win = nn.Linear(control_size, hidden_size, bias=False)
        # Manually set the weights to an identity matrix
        identity_matrix = torch.eye(control_size)
        self.Win.weight = nn.Parameter(identity_matrix, requires_grad=False)
    ...
    def forward(self, u, prev_z):
        # pred_z = self.Win(self.nonlin(u)) + self.Wr(self.nonlin(prev_z))
        pred_z = self.Win(u) + self.Wr(self.nonlin(prev_z))
        pred_x = self.Wout(self.nonlin(pred_z))
        return pred_z, pred_x
    ...
    def update_nodes(self, x, u, prev_z, inf_lr, update_x=False):
        ...
        delta_z = err_z - self.nonlin.deriv(self.z) * torch.matmul(err_x, self.Wout.weight.detach().clone())
        self.z -= inf_lr * delta_z
        # update_x refers to whether x units are clamped or not
        if update_x:
            delta_x = err_x
            x -= inf_lr * delta_x
    ...
    def update_grads(self, x, u, prev_z):
        ...
        err_z, err_x = self.update_errs(x, u, prev_z)
        self.hidden_loss = torch.sum(err_z**2)
        print(f'hidden_loss has value {self.hidden_loss}')
        self.obs_loss_1 = torch.sum(err_x**2)
        print(f'obs_loss before up or down-weighing has value {self.obs_loss_1}')
        # print(f'error_x is of shape {err_x.shape}')
        self.obs_loss = torch.sum(err_x**2) + (self.label_modifier - 1) * (err_x[0, -1]**2) # up/downweigh the error neuron (by default the last sensory neuron) in total energy by label_modifier.
        print(f'obs_loss after up or down-weighing by {self.label_modifier} has value {self.obs_loss}')
        energy = self.hidden_loss + self.obs_loss
        return energy