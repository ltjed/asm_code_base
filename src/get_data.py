...
def load_sequence_mnist(seed, seq_len, order=True, binary=True, train=True):
    ...
    if order:
        ...
        for i in range(seq_len):
            indices = torch.where(mnist.targets == (i % 10))[0]
...
# this function takes in an additional input 'digits' and assign each digit a specific angle
# needs modification to work on more than two digits
def get_rotating_mnist(datapath, seq_len, sample_size, batch_size, seed, angle, digits = None, add_reverse_angled_data=False):
    """digits: list of digits used to train the model
    
    angle: rotating angle at each step
    
    add_reverse_angled_data: if True, make half of sequences with negative angle
    """
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])
    ...
    if digits is None:
        ...
        train_data = train_data[random.sample(range(len(train_data)), sample_size)]
    else:
        num_classes = len(digits)
        # get data from particular classes
        training_data = []
        idx = torch.zeros(len(train.targets), dtype=torch.bool)
        for digit in digits:
            idx = (train.targets == digit).bool()
            digit_data = train.data[idx] / 255.
            digit_data = digit_data[random.sample(range(len(digit_data)), sample_size//num_classes)]
            training_data.append(digit_data)
        train_data = torch.cat(training_data, dim=0)
    ...
    for l in range(seq_len):
        if add_reverse_angled_data:
            train_sequences[:sample_size // num_classes,l] = TF.rotate(train_data[:sample_size // num_classes], angle * l)
            train_sequences[sample_size // num_classes:, l] = TF.rotate(train_data[sample_size // num_classes:], -angle * l)   
        else:
